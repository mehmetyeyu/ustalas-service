...
export function Tooltip({ text, children, align = "right" }: { text: string; children: React.ReactNode; align?: "left" | "right" }) {
  return (
    <span className="relative inline-flex group">
      {children}
      <span className={`pointer-events-none absolute ${align === "left" ? "left-0" : "right-0"} top-full z-50 mt-2 hidden w-64 whitespace-normal rounded-lg bg-gray-900 px-3 py-2 text-xs leading-snug text-white shadow-lg group-hover:block`}>
        {text}
      </span>
    </span>
  );
