...
Lastik, rot ve balans hizmeti veren bir oto servis firması için web tabanlı müşteri, sipariş, stok ve depolama yönetim uygulaması. Admin paneli mobil cihazlarda da tam kullanılabilir (responsive) — tablolar, formlar ve modallar küçük ekranlara uyarlanmıştır. Onay gerektiren aksiyonlar (silme/iptal vb.) uygulama genelinde paylaşılan bir `ConfirmProvider` modalı (`src/components/ConfirmProvider.tsx`, `useConfirm()` hook'u) üzerinden çalışır — tarayıcının native `confirm()`'ü hiçbir yerde kullanılmaz.
...
**Alanlar:**
- **Tarih** — varsayılan bugün, geleceğe kapalı (`max` bugün); geçmiş tarihli girişe izin verir (ör. "2 gün önce girmeyi unuttum"). `POST /api/orders`'a opsiyonel `order_date` olarak gider; sunucu bunu geçerli bir tarihe ve geleceğe ait olmadığına ayrıca doğrular (400), yalnızca **gün** kısmını değiştirir — saat "şimdi" kalır ki aynı gün birden fazla geçmiş sipariş girilirse sıralama bozulmasın. Raporlar `created_at`'e göre gruplandığından bu tek başına siparişin doğru güne düşmesi için yeterlidir.
...
- **"Beni Hatırla"** seçeneği — işaretlenmezse oturum süresi hiç değişmez (JWT `JWT_EXPIRES_IN` + 12 saatlik cookie); işaretlenirse hem JWT hem cookie ömrü 30 güne çıkar (`REMEMBER_ME_EXPIRES_IN`/`REMEMBER_ME_MAX_AGE_SECONDS`, `src/lib/auth.ts` — tek kaynaktan türediğinden ikisi asla birbirinden sapmaz).
...
**Cari durumu canlı hesaplanır (FIFO uzlaşma):** "Ödeme Şekli" sütununda `payment_type = "Cari"` olan bir satır artık statik değil — müşterinin **tüm** Cari hareket geçmişine göre `GET /api/orders`'da sunucu tarafında canlı hesaplanır (`computeOrderLedgerStatus`, bkz. Bölüm 13 — FIFO Uzlaşma). Tamamen ödenmişse hücrede doğrudan ödeme yöntemi gösterilir (ör. "Nakit"; birden fazla farklı yöntemle kapandıysa "Karışık"; yöntem hiç bilinmiyorsa "Cari (Ödendi)"); kısmen ödenmişse `"Cari (₺X kaldı)"` gösterilir. Bu hesap, sayfadaki siparişlerin sahibi olan müşterilerin **tüm** geçmiş kayıtlarını (sayfa dışındaki eski siparişler dahil) çekip tek bir FIFO kuyruğunda işler — Raporlar'daki (Bölüm 6) tahakkuk bazlı "Ödeme Tipine Göre Gelir" kırılımından kasıtlı olarak bağımsızdır, oradaki rakamlar bu canlı uzlaşmadan etkilenmez.

**Ort. Günlük Tutar/Kâr:** Toplam Tutar/Kâr kutucuklarının yanında, aynı filtrelerle seçili aralığın **gerçek çalışma gününe** bölünmüş ortalaması gösterilir (`countWorkingDays`, `src/lib/appointmentSlots.ts`) — ham takvim günü değil, Randevu Ayarları'ndaki (Bölüm 12) Çalışma Saatleri'nde kapalı işaretli günler (ör. Pazar) sayılmaz. Çalışma Saatleri hiç doldurulmamışsa yanlış bir varsayımla (hep 7 gün açık) hesap yapmak yerine "Ort. Günlük için Çalışma Saatlerini girin" linkiyle Randevu Ayarları'na yönlendirilir. Yükleme sırasında bu dört kutucuk da (Toplam Tutar, Kâr, Ort. Günlük Tutar, Ort. Günlük Kâr) tablo satırlarıyla aynı gri skeleton (`animate-pulse`) deseniyle gösterilir.
...
Plaka, sipariş no, statü; müşteri bilgileri; hizmet listesi (her satırda miktar, ebat, tedarikçi, kendi ödeme tipi rozeti); toplam; indirim varsa ayrı satır; notlar; oluşturulma tarihi ve ödeme bilgisi; "Ödeme Al & Kapat" (BEKLEMEDE ise), "İş Emri Yazdır" ve "Düzelt" aksiyonları.

**İş Emri Yazdırma:** `src/app/admin/storage/page.tsx`'teki etiket yazdırma ile AYNI desen — `window.open` + `document.write` + `window.print()`, sunucu round-trip'i veya PDF kütüphanesi yok. Firma bilgisi sipariş anında değil, **yazdırma anında** `GET /api/company-info`'dan çekilir (oturum açmış herkese açık, ekstra bir izin kontrolü yok — sipariş görme izni olan zaten müşteriye basılı olarak gösterilecek bu bilgiyi de görebilir). A4 portrait çıktı: üstte firma logosu (varsa)/adı/adresi/telefonu/vergi dairesi-VKN-TCKN'si ve karşı köşede "İŞ EMRİ" başlığı + sipariş no/tarih; ortada Plaka/Müşteri/Telefon/Durum bilgi ızgarası; hizmet tablosu (Hizmet/Adet/Birim Fiyat/Tutar + Toplam satırı); varsa Not; altta yan yana iki kutu — "Firma Kaşesi" (Firma Kaşesi görseli yüklüyse orada gösterilir, bkz. Bölüm 15 — Marka) ve "Müşteri İmza" (her zaman boş imza kutusu).
...
- Sütun görünürlüğü özelleştirilebilir (`localStorage`) — Üretim Haftası/Mevsim/Tedarikçi/Konum/fiyatların yanında **Ürün Tipi/Barkod/Kondisyon** için de opsiyonel sütunlar vardır (varsayılan kapalı, mevcut kullanıcıların görünümünü değiştirmez).
...
#### Katalog Alanları (Ürün Kataloğu Taslağı — Faz 1)

Müşteri onaylı "Ürün Kataloğu Taslağı"nın Yüksek öncelikli alanları — hepsi nullable/opsiyonel, mevcut ürünleri ve `size_desc`'e bağımlı 20'den fazla yeri (sipariş ekranları, Paylaşılan Stok, Excel, `/api/products/sizes`) hiç değiştirmeden EK olarak eklendi.

- **Ürün Tipi** (`products.product_type`) — Lastik / Jant / İkinci El Lastik / İkinci El Jant / Aksesuar (`src/lib/productCondition.ts` — `PRODUCT_TYPE_OPTIONS`). Seçime göre form kendini uyarlar: Jant/İkinci El Jant/Aksesuar'da Mevsim ve Üretim Haftası/Yılı (lastiğe özel) gizlenir; İkinci El Lastik'te **Diş Derinliği** (mm, `tread_depth_mm`) alanı çıkar ve ondan **Kondisyon** otomatik hesaplanır (7mm ve üzeri "Çok İyi", altı "İyi" — `computeCondition`, saf/test edilmiş fonksiyon, DB'de saklanmaz, elle girilmez). NULL `product_type` için eski davranış (hep Mevsim/Üretim görünür, Diş Derinliği hiç görünmez) korunur.
- **Ebat (yapılandırılmış giriş)** — mevcut tek-kutu `size_desc` alanının YANINA, opsiyonel olarak Kesit Genişliği/Profil/Jant Çapı (`width_mm`/`profile_pct`/`rim_diameter`) ayrı ayrı girilebilir; üçü de doluysa `size_desc` otomatik "225 / 45 / R17" biçiminde oluşturulur (elle de düzeltilebilir).
- **Barkod** (`products.barcode`, firma içinde benzersiz) — girilip alandan çıkılınca (`onBlur`, okuyucular genelde Tab/Enter simüle eder) o firmanın **daha önce aynı barkodla kaydettiği** bir ürün varsa Marka/Ebat/Mevsim/Tedarikçi/Ürün Tipi/Ebat otomatik doldurulur (sadece henüz elle doldurulmamış alanlara — dolu bir alanın üzerine sessizce yazılmaz); kontrol sürerken alanın içinde küçük bir dönen gösterge belirir (`/api/products/barcode`). Dış bir barkod veritabanına bağlı DEĞİLDİR (güvenilir bir kaynak bulunamadı, araştırıldı) — yalnızca firmanın kendi geçmiş ürünlerini hatırlar, ilk kez görülen bir barkodda hiçbir şey doldurmaz. Fiyat/stok/konum kasıtlı olarak doldurulmaz (bunlar her partiye özgüdür). Hem Yeni Ürün hem Düzenle formunda çalışır.
- **Benzer Üründen Kopyala** — her partinin satırında bir "Kopyala" butonu; tıklanınca Marka/Ebat/Mevsim/Tedarikçi/Konum/fiyatlar (Ürün Tipi ve varsa yapılandırılmış Ebat dahil) yeni "Ürün Ekle" formuna taşınır — Ürün Kodu, Barkod, Üretim Haftası/Yılı, Stok Miktarı ve Diş Derinliği KASITLI olarak taşınmaz (bunlar o partiye/spesifik ürüne özeldir).
- **Ön İzleme** — form doldurulurken altında, ürünün listede nasıl görüneceğini (rozetler, marka/ebat, kod/tedarikçi/konum, fiyat) gösteren canlı bir kart belirir; ayrı bir state tutmaz, formdan türetilir.
- Taslağın Orta/Düşük öncelikli alanları (Model/Ürün Hattı, Yük/Hız Endeksi, AB Lastik Etiketi, Jant'a özel ölçü/PCD/ET, Minimum Stok Eşiği) bilinçli olarak ayrı bir faza bırakıldı, henüz eklenmedi.
...
- **İçe aktarma öncesi doğrulama:** Mevsim gibi sınırlı değer kümesi olan bir sütunda tanınmayan bir değer varsa (yazım hatası vb.), dosya sisteme yüklenmeden önce satır/hücre bazlı bir uyarı diyaloğu gösterilir — kullanıcı isterse "Yine de Devam Et"le geçebilir. İndirilen şablonda ayrıca geçerli değerleri listeleyen bir "Geçerli Değerler" referans sayfası bulunur (`xlsx` kütüphanesinin ücretsiz sürümü native dropdown/veri doğrulama yazamadığından bu şekilde çözüldü).
...
**FIFO Uzlaşma — bir siparişin Cari'si ne zaman "ödendi" sayılır:** `computeOrderLedgerStatus` (`src/lib/customerLedger.ts`) bir müşterinin **kronolojik sıradaki tüm** `customer_ledger_entries` kayıtlarını tek bir geçişte işleyip her borç (`direction=1`) kaydına bir "kuyruk" mantığıyla en eski borçtan başlayarak tahsilat (`direction=-1`) düşer — klasik esnaf Cari defteri modeli: bir tahsilat belirli bir siparişe değil, kuyruktaki **en eski** açık borca düşer (çoğu MANUEL tahsilat kaydının zaten hangi siparişe ait olduğu belli değildir, `order_id` boştur). Algoritma:
- Her borç kaydı (SIPARIS veya MANUEL, `order_id` boş olsa bile) kuyruğun sonuna `{orderId, kalan: tutar}` olarak eklenir.
- Her tahsilat kaydı, kuyruğun **en başındaki** (en eski) borcu bitirene kadar (o borcu tam kapatıp bir sonrakine geçerek) veya tahsilat tutarı tükenene kadar sırayla düşülür — böylece tek bir büyük tahsilat, art arda birden fazla eski siparişi tek geçişte kapatabilir. Bir borcu kapatmaya katkısı olan her tahsilatın `payment_type`'ı o sipariş için ayrıca biriktirilir.
- Sonuçta her `order_id`'li borç için: `remainingAmount = 0` ise tamamen ödenmiş (`paidVia`: tek bir ödeme tipi katkı yaptıysa o tip, birden fazla farklı tipse `"Karışık"`), `0 < remainingAmount < originalAmount` ise kısmen ödenmiş, `remainingAmount = originalAmount` ise hiç dokunulmamış.
- Bu fonksiyon iki yerde kullanılır: (1) Sipariş Listesi'ndeki canlı "Cari" durumu (bkz. Bölüm 3) — `GET /api/orders`, sayfadaki siparişlerin müşterilerine ait TÜM ledger kayıtlarını (sadece sayfadaki siparişlerinkini değil) çekip müşteri bazında gruplar, sonucu `cari_settled`/`cari_remaining_amount`/`cari_paid_via` alanlarıyla `items`'a ekler; (2) aşağıdaki Cari Hareketleri modalının kendisi değil, ama aynı prensip — bakiye her zaman kümülatif/canlıdır.
...
**Cari Hareketleri modalı — sütunlar** (genişletilmiş hâli, `GET /api/customers/:id/ledger`): Tarih · Sipariş No (SIPARIS için sipariş detayına link, MANUEL için "—") · Plaka · Hizmet/Açıklama (SIPARIS'te her satırın adı×adedi + varsa notlar; MANUEL'de not veya "Tahsilat (yöntem)"/"Borç") · Ürün (SIPARIS'te ebat + tedarikçi, MANUEL'de "—") · Tutar (yön işaretli, siparişin kendisi tam ödenmemişse altında turuncu "Sipariş: ₺X / Alınan: ₺Y" alt satırı) · Bakiye (kümülatif) · aksiyon sütunu (yalnızca `manage_balance` izniyle, yalnızca MANUEL kayıtlarda Düzenle/Sil — SIPARIS kayıtları yalnızca ilgili siparişin kendisi değiştirilerek dolaylı güncellenir).
...
- Depolama modülündeki (Bölüm 8) "N aydan uzun süredir bekliyor" uyarısı hem liste sayfasında hem `/api/storage?overdue=true` sorgusunda artık bu ayardan okunur (önceden kod içinde sabit 6 ay idi). Bu alan artık sayfanın en altında, kendi ayrı **"Depolama"** kartındadır (önceden Firma Kodu'nun hemen altındaydı).
- **"Marka" kartı** — Firma Logosu, Panel Logosu, Firma Kaşesi görsel yüklemeleri (`ImageUploadField` bileşeni, PNG/JPG/WEBP/SVG, azami 2MB) — `POST/DELETE /api/company-info/assets` ile Vercel Blob'a **public** erişimle yüklenir (`tenants.logo_url`/`panel_logo_url`/`stamp_url`); her yüklemede zaman damgalı YENİ bir dosya yolu kullanılır (üzerine yazılmaz — tarayıcı önbelleğinde eski görselin takılı kalmasını önlemek için, Vercel'in "blobları immutable kabul et" önerisine uyarak), eski blob DB güncellendikten SONRA best-effort silinir. **Firma Logosu** İş Emri çıktısının başlığında (bkz. Bölüm 4), **Firma Kaşesi** aynı çıktının "Firma Kaşesi" kutusunda, **Panel Logosu** ise admin panel header'ında (masaüstü + mobil) `business_name` metninin YERİNE gösterilir — ayarlanmamışsa eskisi gibi metin kalır.
- **"Şirket Bilgisi" kartı** genişletildi: mevcut Yetkili Adı/E-posta/Cep Telefonu'na ek olarak **Sabit Telefon** (`tenants.landline_phone`) ve **Web Sitesi** (`tenants.website`) eklendi — bu kart Paylaşılan Stok eşleşmesinde (Bölüm 19) karşı firmaya gösterilen, düşük riskli iletişim bilgisi kategorisidir.
- **"Fatura Bilgileri" kartı** (VKN/TCKN, Vergi Dairesi, Fatura Unvanı, İl/İlçe/Adres) — KASITLI olarak Şirket Bilgisi kartından AYRI (cross-tenant görünür kategoriyle karışmasın diye, bu alanlar asla Paylaşılan Stok'ta gösterilmez). Bu form önceden Profil sayfasındaydı, firma çapında bir bilgi olduğundan buraya taşındı; Abonelik sayfasındaki (bkz. Bölüm 20) aynı formla (`GET/PUT /api/billing/invoice-info`) birebir aynı veriyi okur/yazar — biri kaydedince diğeri de güncel görünür.
...
**Panel:**
- Tüm gerçek firmaları (`is_platform = false`) Firma Adı, İletişim (bkz. Bölüm 18), Firma Kodu, Kayıt Tarihi, Aktif/Pasif durumu ve **Faturalandırma** sütunuyla listeler (`GET /api/super-admin/tenants`, artık `{ tenants, usdTryRate }` döner). Faturalandırma sütunu: durum rozeti (Deneme Sürümü/Aktif/Ödeme Sorunu/İptal Edildi/Muaf — bkz. Bölüm 20) + plan etiketi (Aylık/Yıllık); `active` ve iptale girmemişse "Yenileme: {tarih}", `billing_cancel_at_period_end=true` ise turuncu "İptal — {tarih}'e kadar erişim", `past_due` ise `billing_last_payment_error`'daki gerçek iyzico red mesajı kırmızı gösterilir.
- **Özet kartları** (dört kutu): **Aktif Abone (Aylık)** / **Aktif Abone (Yıllık)** (fiilen aktif sayımı — `active` VE (iptale girmemiş VEYA dönem sonu henüz gelmemiş)); **Tahmini Aylık Gelir (MRR)** — yalnızca VİTRİN fiyatı (`USD_REFERENCE_PRICING`, bkz. Bölüm 20) üzerinden, yıllıklar 12'ye bölünüp aylığa normalize edilerek, güncel TCMB kuruyla TL'ye çevrilir (gerçek iyzico plan fiyatları tek tek çekilmez, gösterge amaçlıdır); **"Bu Ay Gerçek Net Gelir"** — aşağıya bkz.
...
- **"+ Yeni Firma Ekle"** (`POST /api/super-admin/tenants`) — `scripts/create-tenant.mjs`'in çağırdığı **aynı** `src/lib/provisionTenant.ts`'i doğrudan çağırır (`startTrial` göndermez, dolayısıyla oluşan firma her zaman `billing_status = 'exempt'` olur — iyzico'ya hiç girmez); oluşturulan Firma Kodu ve Kullanıcı Adı, kapanınca kaybolmaması için modalda ayrıca tutulur (müşteriye iletilecek bilgi).
...
- **Arama** — Firma Adı, Firma Kodu, Yetkili Adı/E-posta/Telefon üzerinde tek bir metin kutusuyla istemci tarafında filtrelenir.
- **"Tutarlılığı Kontrol Et"** (`GET/POST /api/super-admin/consistency-check`) — DB↔iyzico abonelik tutarlılık denetimi, bkz. Bölüm 20 "Repricing ve Tutarlılık Denetimi".
- **Ödeme Geçmişi modalı** ("Ödemeler" linki, tenant satırından) — `GET /api/super-admin/tenants/:id/payments`, iyzico'nun Raporlama Servisi'nden (`getPaymentDetailsByConversationId`, `conversationId` = tenant id — her checkout/switch-plan'da AYNI gönderildiğinden, aboneliğin `customerReferenceCode`'u zaman içinde değişse bile TAM geçmiş görünür) çekilir. Sütunlar: Tarih, **Ödeme No**, Tutar, **Komisyon** (iyzico'nun kestiği, kırmızı `-` işaretli), Net Gelir (`merchantPayoutAmount`), Durum (yalnızca gözlemlenen tek gerçek değer "Başarılı"; tanınmayan ham değer olduğu gibi gösterilir), **Hesap Durumu** — alttaki komisyon+net gelir toplam satırıyla birlikte. `fraudStatus` yalnızca ANORMAL değerlerde (0=İnceleniyor turuncu, 2=İncelendi/Onaylandı mavi, -1/-2/-3=Reddedildi kırmızı) bir rozet olarak gösterilir; en sık görülen değer olan 1 (temiz) için hiç rozet gösterilmez. **Hesap Durumu** (`blockageResolvedDate`) — iyzico başarılı bir tahsilatı hemen değil, bir "blokaj" (hold) süresinin (gerçek veride ~8 gün) sonunda banka hesabına aktarır; tarih geçmişse "Hesapta" (yeşil), gelecekteki bir tarihse "Bekliyor (tarih)" (turuncu). Bu alan yalnızca bu modalda vardır — aylık özet kartındaki "Gerçek Net Gelir" bunu ayıramaz, çünkü onun kullandığı gün-bazlı Raporlama Servisi ucu bu alanı döndürmez.
- **"Bu Ay Gerçek Net Gelir"** (`GET /api/super-admin/revenue?month=YYYY-MM`) — yukarıdaki vitrin-fiyatlı MRR tahmininden tamamen ayrı, iyzico Raporlama Servisi'nden GÜN GÜN (bu uç aralık değil tek tarih kabul eder) çekilip toplanan gerçek işlemlere dayanır; sadece `transactionType = "PAYMENT"` olanlar sayılır, `merchantPayoutAmount`/`paidPrice`/komisyon toplanır. Günler `Promise.all` ile hepsi birden değil, `CONCURRENCY=2` ile küçük gruplar halinde (aralarında 400ms bekleyerek) ve başarısız bir gün için üstel geri çekilmeli (2s/4s/8s) tekrar denemeyle çekilir — tüm günlerin aynı anda çekilmesi gerçek bir denemede iyzico'nun (sandbox'ta bile düşük olan, ~18-20 eşzamanlı istekte tetiklenen) rate limit'ine takılmıştı; bir gün tüm denemelerde başarısız olursa o gün 0 katkı sayılıp devam edilir (tek günün hatası tüm ayı düşürmesin diye).
- **Faturalandırma Olay Günlüğü** ("Son Faturalandırma Olayları" tablosu, `GET /api/super-admin/billing-events`, `billing_events` tablosundan en yeni 100 kayıt) — webhook başarılı/başarısız, eşleşmeyen webhook/IFN, iptal edilmiş abonelikte yoksayılan yenileme, repricing başarılı/başarısız, tutarlılık düzeltmesi gibi olayları Tarih/Firma/Olay (renk kodlu rozet)/Detay sütunlarıyla gösterir — önceden bu olaylar yalnızca sunucu loglarına (`console.warn`/`error`) düşüyordu, panelde hiç görünmüyordu.
...
- **Elevire landing:** tüm CTA'lar `/kayit`'a giden "Ücretsiz Hesap Oluştur"/"7 Gün Ücretsiz Dene" ile birleştirildi. Paylaşımlı Elevire demo hesabı (otomatik doldurulan sabit Firma Kodu/Kullanıcı Adı/Şifre, gecelik sıfırlama mekanizmasıyla) TAMAMEN kaldırıldı — hem landing'deki "Demoyu Ücretsiz Dene" CTA'ları ve üst menüdeki "Demoyu Dene" linki, hem giriş formunun `NEXT_PUBLIC_DEMO_MODE` ile koşullu ön-doldurması (giriş formu artık her zaman boş başlar); `/kayit` üzerinden gerçek, kartsız 7 günlük deneme (bkz. Bölüm 20) artık aynı ihtiyacı karşılıyor.
...
### 20. Abonelik (SaaS Faturalandırma) — iyzico Entegrasyonu

**Amaç:** `/kayit`'tan kendi kendine kayıt olan firmalar (Bölüm 18) artık 7 gün kartsız deneme sonrası Aylık/Yıllık bir iyzico aboneliğine geçiyor; ödeme kesilir veya deneme biterse erişim `/admin/billing`'e kilitlenir. Mevcut/manuel firmalar (Ustalas, FB Lastik, Süper Admin panelinden "+ Yeni Firma Ekle" ile eklenenler) bu mekanizmadan tamamen **muaf**tır — hiçbir şekilde etkilenmezler. Kart bilgisi hiçbir zaman bu sunucuya dokunmaz, iyzico'nun barındırdığı Checkout Form'da girilir.

**Faturalandırma durumu ve erişim kilidi:** `tenants.billing_status` beş değerden biridir — `exempt` (manuel/eski firmalar, süresiz muaf), `trialing` (deneme süresinde, `trial_ends_at`), `active` (geçerli abonelik), `past_due` (son tahsilat başarısız/reddedildi), ve arayüzde türetilen (DB'de hiç yazılmayan, sadece görüntü seviyesinde hesaplanan) `canceled` etiketi. Kilit mantığı TEK bir fonksiyonda toplanır — `isBillingLocked()` (`src/lib/billing.ts`): `exempt` asla kilitlenmez; `active` yalnızca iptale girmiş (`billing_cancel_at_period_end=true`) VE ödenmiş dönem sonu (`billing_period_ends_at`) geçmişse kilitlenir (Netflix tarzı — iptal edilen bir abonelik parasını ödediği sürenin sonuna kadar çalışmaya devam eder); `trialing` yalnızca `trial_ends_at` geçmişse; diğer her durum (`past_due`, `canceled`, `null`) koşulsuz kilitlidir. Bu, `tenants.is_active`'ten (Bölüm 16/17) KASITLI olarak ayrı bir mekanizmadır: `is_active=false` girişin kendisini engeller, faturalandırma kilidinde ise kullanıcı giriş yapabilmeli, `/admin/billing`'e ulaşıp kendi kendine yeniden abone olabilmelidir.

Kilit `src/middleware.ts`'te **üç ayrı yerde** uygulanır: (1) `/admin/*` sayfa yönlendirmesi (kilitliyse `/admin/billing` dışındaki her sayfadan oraya atılır), (2) `/` kök yönlendirmesi, (3) **API katmanının kendisi** — `/api/*` isteklerinde de aynı kontrol yapılıp kilitliyse 402 + Türkçe hata JSON'ı dönülür (`/api/public/*`, `/api/auth/*`, `/api/billing/*`, `/api/webhooks/*`, `/api/super-admin/*` hariç — bunlar kimlik doğrulaması farklı/yok veya kilitli bir firmanın kilitten çıkabilmesi için gerekli). Üçüncü katman sonradan eklendi: öncesinde kilitli bir firma sayfa yönlendirmesini atlayıp (eski bir sekme, doğrudan `curl`) API'lere erişmeye devam edebiliyordu.

**Sayfa erişimi:** `/admin/billing` izin sisteminde `resource: null` ile işaretlidir (Profil gibi) — herhangi bir oturum açmış kullanıcı (staff dahil) sayfayı açabilir, çünkü kilitli bir tenant'ın middleware yönlendirmesi herkesi (role fark etmeksizin) buraya atar. Ama gerçek abonelik yönetimi (checkout/switch-plan/cancel/invoice-info) API katmanında `admin` rolüyle sınırlıdır — `staff` sayfayı açtığında yalnızca "Bu firmanın aboneliği güncel değil, firma yöneticinizle iletişime geçin" bilgi mesajı görür, plan kartları/checkout UI'ı hiç render edilmez.

**Checkout akışı** (`/admin/billing`, `admin` rolü):
1. `GET /api/billing/plans` — hem Aylık hem Yıllık planın canlı iyzico fiyatını (`getPricingPlan`) hem güncel TCMB USD/TRY kurunu döner.
2. Ödeme öncesi iki ön koşul UI'da devre dışı bırakır: **Fatura Bilgileri** eksikse (bkz. aşağı) ve **Mesafeli Satış Sözleşmesi** onay kutusu işaretlenmemişse "Abone Ol"/"Bu Plana Geç" butonları pasif kalır.
3. `POST /api/billing/checkout` (`plan: "monthly"|"yearly"`) — `acceptedTerms` zorunlu (anında `terms_accepted_at` yazılır, sonrasında iyzico çağrısı başarısız olsa bile — onay o anda verilmiştir); `tenants.billing_checkout_lock_at` üzerinde tek bir atomik `UPDATE ... RETURNING` ile **eşzamanlı checkout kilidi** alınır (15 dakika TTL, `switch-plan` ile paylaşılır) — aynı firmadan art arda/çift tıklamayla iki paralel abonelik açılmasını engeller; fatura bilgisi eksikse kilit hemen serbest bırakılıp 400 döner; firma `past_due` ise (kötü karta bağlı eski abonelik iyzico'da kendiliğinden iptal olmaz) önce best-effort eski abonelik iptal edilir — aksi halde eski kart sonradan çalışır hale gelirse sürpriz mükerrer tahsilat riski vardı; hâlâ deneme süresindeyse kalan gün sayısı `trialPeriodDays` olarak iyzico'ya geçilip erken abone olanlar kalan ücretsiz süreyi kaybetmez.
4. iyzico'nun döndürdüğü Checkout Form içeriği (saf `<script>` etiketleri — `innerHTML` ile eklenirse çalışmaz, bu yüzden her `<script>` `document.createElement` ile yeniden oluşturulup eklenir) sayfaya **gömülü/responsive** olarak render edilir (`#iyzipay-checkout-form`) — bu, popup → sağdan kayan sidesheet (denenip geri alındı) → gömülü denemelerinin son hâlidir. Token, `iyzico_checkout_sessions` tablosuna tenant_id ile eşlenerek kaydedilir — çünkü checkout form yanıtı gönderilen `conversationId`'yi HİÇ geri döndürmez, callback tenant'ı bu tablodan bulur.
5. `GET/POST /api/billing/callback` — token'dan tenant'ı `iyzico_checkout_sessions`'tan bulur, `subscriptionStatus === "ACTIVE"` ise aktive eder (`billing_status='active'`, `billing_subscription_ref`, `billing_period_ends_at` = yerel olarak +1 ay/+1 yıl hesaplanır); zaten aynı referansla `active` ise (replay/çift callback) tekrar işlemez. Checkout kilidi `finally` bloğunda her durumda serbest bırakılır.

**Plan değiştirme ve iptal:** `switch-plan` iyzico'nun native (prorasyonlu) upgrade'i DEĞİLDİR — mevcut abonelik hemen iptal edilip yeni plan **tam fiyattan** yeniden checkout'a girer, kalan süre için mahsup/iade yoktur. Bu yüzden "Bu Plana Geç" butonu yalnızca **yenileme tarihine 3 gün kala** gösterilir (repricing cron'uyla aynı pencere, aşağıya bkz.) — öncesinde kart yalnızca bilgilendirme metni gösterir, üstteki sözleşme onay kutusu da o an hiçbir aksiyon alınamayacağından gizlenir. **İptal** (`POST /api/billing/cancel`) iyzico'da anında gerçekleşir (bir daha tahsilat olmaz) ama `billing_status='active'` kalır, yalnızca `billing_cancel_at_period_end=true` set edilir — erişim ödenmiş dönemin sonuna kadar sürer.

**Fiyatlandırma ve kur:** Vitrin fiyatı (`$25`/ay, `$250`/yıl) TEK bir kaynaktan gelir — `USD_REFERENCE_PRICING` (`src/lib/exchangeRate.ts`) — hem Elevire landing hem `/admin/billing` aynı sabiti okur (önceden iki dosyada birbirinden habersiz iki ayrı sabit vardı). Gerçek tahsilat her zaman **TL**'dir (Türk kartları döviz tahsilatını desteklemez) — iyzico planları sabit TL fiyatla oluşturulur (`scripts/iyzico-setup.mjs`, tek seferlik kurulum, `currencyCode` parametreli), zamanla kur karşısında aşınır. `getUsdTryRate()` aynı dosyada TCMB'nin resmi, ücretsiz kur feed'inden (`tcmb.gov.tr/kurlar/today.xml`, satış/`ForexSelling` kuru, regex ile ayrıştırılır, 6 saat cache) çekilir; `/admin/billing`'de büyük `$X` vitrin fiyatının altında küçük "Kartınızdan ₺Y tahsil edilecek" satırıyla birlikte gösterilir — ikisinin zamanla birbirinden sapması bilinçli olarak kabul edilmiştir.

**Repricing cron'u:** Vercel Cron, her gün 06:00 UTC'de `GET /api/cron/reprice-subscriptions`'ı çağırır (`vercel.json`, `CRON_SECRET` Bearer header ile korunur — gerçek tahsilat fiyatını değiştiren KRİTİK bir uç). `billing_status='active'`, iptale girmemiş, `billing_period_ends_at`'i önümüzdeki 3 gün içinde olan ve o dönem için henüz repriced edilmemiş (`billing_repriced_for_period_end`) her tenant için: o günün TCMB kuruyla yeni bir TL fiyatı hesaplanır, iyzico'da (plan fiyatları DEĞİŞTİRİLEMEDİĞİNDEN) her zaman **yeni bir** pricing plan nesnesi oluşturulup `upgradeSubscription(ref, yeniPlan, "NEXT_PERIOD")` ile yalnızca bir sonraki tahsilata uygulanır — mevcut (zaten ödenmiş) dönem etkilenmez, kart yeniden girilmez. Guard: mevcut planın gerçekten `IYZICO_PRODUCT_REF`'e ait olduğu önceden doğrulanır (iyzico'nun upgrade'i yalnızca aynı ürünün planları arasında çalışır, aksi halde belirsiz bir hatayla başarısız olurdu). iyzico çağrısı başarılı olup DB yazması başarısız olursa (nadir ama Neon soğuk başlangıcı vb. ile mümkün) özel bir KRİTİK log bırakılır — aksi halde `tenants.billing_subscription_ref` bayat bir referansa işaret etmeye devam eder ve gelecekteki hiçbir webhook o tenant'a eşleşmez (bkz. aşağıdaki Tutarlılık Denetimi).

**Repricing ve Tutarlılık Denetimi** (Süper Admin, bkz. Bölüm 17 "Tutarlılığı Kontrol Et"): `GET/POST /api/super-admin/consistency-check` her `active` tenant için DB'deki `billing_subscription_ref`'in iyzico'da gerçekten `ACTIVE` olup olmadığını doğrular; uyumsuzluk bulunursa `listSubscriptions()` ile aynı `parentReferenceCode` altındaki gerçek aktif kardeş aboneliği arayıp tek bir aday varsa `suggestedFix` olarak önerir — otomatik yazmaz, Süper Admin'in POST ile onaylaması gerekir, o zaman da sunucu tarafında önerilen referansın hâlâ `ACTIVE` olduğu tekrar doğrulanır.

**Webhook (abonelik yaşam döngüsü):** `POST /api/webhooks/iyzico` — `subscription.order.success`/`subscription.order.failure` olaylarını dinler. `IYZICO_WEBHOOK_SIGNATURE_ENABLED=true` olmadan (iyzico entegrasyon ekibi tarafında imza doğrulaması açılana kadar) hiçbir DB yazması yapmaz — sahte bir "ödeme başarılı" isteğiyle kilit açtırmak mümkün olmasın diye. Açıldığında `X-IYZ-SIGNATURE-V3` (HMAC-SHA256) doğrulanır; `iyzico_webhook_events` tablosuna `order_reference_code` PRIMARY KEY ile `INSERT ... ON CONFLICT DO NOTHING` yazılarak **idempotency** sağlanır (webhook sağlayıcıları aynı olayı birden fazla kez gönderebilir — tekrar teslimat dönem sonunu bir daha uzatmaz). Başarılıysa dönem bir sonraki periyoda uzatılır ve `iyzico_payments` tablosuna (paymentId→tenant eşlemesi, IFN için gerekli, aşağıya bkz.) best-effort bir kayıt düşülür — ama abonelik zaten `billing_cancel_at_period_end=true` ise dönem BİLEREK uzatılmaz (iptal edilmiş bir abonelik yanlışlıkla yeniden kilitsiz kalmasın diye), bu durum da ayrıca loglanır. Başarısızsa (V1'de otomatik yeniden deneme/dunning YOK) tenant hemen `past_due`'ya kilitlenir; webhook'un kendisi red sebebini içermediğinden `GET /v2/subscription/subscriptions/{ref}`'in `paymentAttempts`'inden AYRI bir çağrıyla gerçek iyzico red mesajı çekilip `billing_last_payment_error`'a yazılır ve `/admin/billing`/Süper Admin'de gösterilir. `subscriptionReferenceCode` hiçbir tenant'a eşleşmezse (bkz. yukarıdaki repricing/DB tutarsızlığı riski) sessizce yutulmaz, `billing_events`'e (Bölüm 17 — Faturalandırma Olay Günlüğü) loglanır.

**IFN (Instant Fraud Notification):** `POST /api/webhooks/iyzico-fraud/[secret]` — iyzico'nun Checkout Form/Abonelik dahil TÜM ürünleri kapsayan ayrı bir dolandırıcılık bildirimi mekanizması (iyzico entegrasyon ekibiyle teyit edilmiştir); ayrı bir imza şeması yok, bu yüzden URL'in kendisi gizli tutulur (`[secret]` segmenti `IYZICO_IFN_WEBHOOK_SECRET` ile karşılaştırılır, yalnızca iyzico Merchant Panel'inde tanımlanır). Payload sadece `{paymentId, fraudStatus}` taşır; `fraudStatus` -3/-2/-1 = reddedildi (işlem gerektirir), 0/1/2 = bilgilendirme (dokunulmaz). `paymentId`→tenant eşlemesi `iyzico_payments` tablosundan yapılır (iyzico'nun `/payment/detail`'i orijinal `conversationId`'yi geri vermediğinden bu eşlemenin kendi tarafımızda ayrıca tutulması gerekir). Reddedilen bir ödemede tenant hemen `past_due`'ya kilitlenir ve best-effort abonelik de iptal edilir (iyzico'nun bu reddi aboneliği otomatik iptal edip etmediği dokümante edilmemiştir).

**Eşzamanlılık/güvenilirlik denetimlerinden çıkan diğer düzeltmeler:** checkout, `past_due` bir firma yeniden abone olurken eski (kötü karta bağlı, iyzico'da hâlâ ACTIVE) aboneliği artık best-effort iptal ediyor — aksi halde iki paralel abonelik açılabiliyordu; callback, aynı referansla zaten `active` bir aboneliğe tekrar gelen bir isteği (replay) dönem uzatmadan başarılı sayıyor.

**Fatura Bilgisi Toplama (VUK md. 231/5):** İlk tahsilattan itibaren 7 gün içinde e-Fatura/e-Arşiv kesme zorunluluğu vardır; bunun için gereken alanlar (`tenants.billing_entity_type` "individual"/"company", `billing_tax_id` — Şahısta 11 haneli TCKN/Şirkette 10 haneli VKN, `billing_tax_office`, `billing_invoice_title`, `billing_city`, `billing_district`, `billing_address`) checkout'a giden sabit placeholder adresin yerine gerçek veriyle dolar. TEK doğruluk kaynağı `isInvoiceInfoComplete()` (`src/lib/billing.ts`); hem kaydetme ucu (`GET/PUT /api/billing/invoice-info`) hem `POST /api/billing/checkout`/`switch-plan` (ödeme öncesi SUNUCU tarafında zorunlu kontrol — asıl güvenlik burada, UI sadece yardımcı) aynı fonksiyonu kullanır. Aynı `InvoiceInfoForm` bileşeni hem `/admin/billing`'de (yalnızca bir checkout/switch-plan aksiyonu mümkünken gösterilir) hem Genel Ayarlar'da (Bölüm 15 — "Fatura Bilgileri" kartı, firma çapında bir bilgi olduğundan kişisel Profil sayfası yerine buraya taşındı) görünür; biri kaydedince diğeri otomatik güncel görünür.

**Telefon numarası normalizasyonu:** iki katmanlı ve KASITLI olarak ayrı. `src/lib/phone.ts` — `/kayit` kaydında ve Genel Ayarlar'da (Sabit Telefon dahil) kaynağında doğrulanıp `"0XXX XXX XX XX"` kanonik biçimine normalize edilir (`normalizeTurkishPhone`, geçersizse kayıt reddedilir); alan yazılırken de aynı biçimde canlı formatlanır (`formatTurkishPhoneInput`, sadece görsel, doğrulama yapmaz). `src/lib/iyzico.ts`'teki AYRI bir `normalizeGsmNumber()` ise bu kanonik formatı checkout'a giderken E.164'e (`+90XXXXXXXXXX`) çevirir — iyzico yerel formatı kabul etmez; tanınmayan bir biçim varsa olduğu gibi geçilir (sessizce yanlış tahmin etmek yerine iyzico'nun kendi hatası yüzeye çıksın diye).

**Sözleşmeler / Yasal Sayfalar** (`src/app/elevire/legal/*`, gerçek şirket bilgileriyle — Yerden Yüksek Yazılım ve Tasarım Teknolojileri Limited Şirketi): Hakkımızda, Gizlilik Sözleşmesi (KVKK), Mesafeli Satış Sözleşmesi, İptal/İade Koşulları. Mesafeli Satış Sözleşmesi'nin fiyat maddesi: ücretler USD referanslı belirlenir (altyapı maliyetleri döviz cinsinden olduğundan) ama TL tahsil edilir; TL karşılığı kur önemli değiştiğinde dönem başında yeniden hesaplanabilir, güncel tutar her zaman `/admin/billing`'de görülebilir (bir önceki taslaktaki somut "15 gün önce e-posta bildirimi" taahhüdü, gerçekte inşa edilmeyen bir SLA'ya sözleşmeyle bağlanmamak için kaldırıldı). Kayıt/ilk ödeme akışında ayrı onay checkbox'ları vardır: `/kayit`'ta Gizlilik (`privacy_policy_accepted_at`), `/admin/billing`'de her checkout/switch-plan öncesi Mesafeli Satış Sözleşmesi (`terms_accepted_at`) — her ikisi de sunucu tarafında zorunludur. Dijital hizmetin anlık ifası nedeniyle cayma hakkı yoktur (Mesafeli Sözleşmeler Yönetmeliği md. 15), bunun karşılığında kartsız 7 günlük deneme sunulur.

**Deneme süresi uyarıları:** `trialing` bir admin'e üstte kalıcı bir amber banner'a EK olarak, sayfa geçişlerinde ara sıra (sekme başına sayaç — ilk geçişte + her 5. geçişte bir, `sessionStorage`, `/admin/billing`'in kendisinde hiç gösterilmez) "Deneme Süreniz Bitmek Üzere" modalı çıkar ("Daha Sonra" / "Abone Ol").

**Şema (`tenants` üzerindeki tüm faturalandırma kolonları):** `plan`, `billing_provider`, `billing_customer_id`, `billing_status`, `trial_ends_at` (multi-tenant dönüşümünde ayrılmış, bu modülle ilk kez kullanılmaya başlandı), `billing_subscription_ref`, `billing_period_ends_at`, `billing_cancel_at_period_end`, `billing_checkout_lock_at`, `billing_pricing_plan_ref`, `billing_repriced_for_period_end`, `billing_last_payment_error`, `privacy_policy_accepted_at`, `terms_accepted_at`, `billing_entity_type`/`billing_tax_id`/`billing_tax_office`/`billing_invoice_title`/`billing_city`/`billing_district`/`billing_address` (fatura bilgileri). Yeni tablolar: `iyzico_checkout_sessions`, `iyzico_webhook_events`, `iyzico_payments`, `billing_events` — hepsi Veritabanı Şeması bölümünde ayrıca listelenir.
...
# iyzico Abonelik (SaaS faturalandırma, bkz. Bölüm 20) — sandbox:
# https://sandbox-api.iyzipay.com, production: https://api.iyzipay.com
IYZICO_API_KEY=
IYZICO_SECRET_KEY=
IYZICO_BASE_URL=https://sandbox-api.iyzipay.com
# scripts/iyzico-setup.mjs çıktısından (tek seferlik kurulum)
IYZICO_PRODUCT_REF=
IYZICO_PLAN_MONTHLY_REF=
IYZICO_PLAN_YEARLY_REF=
# X-IYZ-SIGNATURE-V3 webhook imzası, iyzico entegrasyon ekibi tarafından
# açılana kadar bunlar boş/false kalmalı — açık değilse webhook hiçbir
# state değişikliği yapmaz.
IYZICO_MERCHANT_ID=
IYZICO_WEBHOOK_SIGNATURE_ENABLED=false
# IFN (Instant Fraud Notification) — gizli webhook URL segmenti
IYZICO_IFN_WEBHOOK_SECRET=
# Vercel Cron'un reprice-subscriptions'ı çağırırken gönderdiği Bearer token
CRON_SECRET=
```
...
| Tablo | Amaç |
|---|---|
| `tenants` | Firmalar (çoklu firma altyapısı, Bölüm 16) — `name`, `slug`, `code` (6 haneli Firma Kodu), `is_active`, `is_platform` (yalnızca dahili "Platform Yönetimi" tenant'ında `true` — Süper Admin hesaplarını barındırır, bkz. Bölüm 17), `contact_name`/`contact_email`/`contact_phone`/`landline_phone`/`website` (Bölüm 15/18/19), `logo_url`/`panel_logo_url`/`stamp_url` (Bölüm 4/15 — Vercel Blob), faturalandırma (Bölüm 20): `plan`, `billing_provider`, `billing_customer_id`, `billing_status`, `trial_ends_at`, `billing_subscription_ref`, `billing_period_ends_at`, `billing_cancel_at_period_end`, `billing_checkout_lock_at`, `billing_pricing_plan_ref`, `billing_repriced_for_period_end`, `billing_last_payment_error`, `privacy_policy_accepted_at`, `terms_accepted_at`, `billing_entity_type`/`billing_tax_id`/`billing_tax_office`/`billing_invoice_title`/`billing_city`/`billing_district`/`billing_address` (Fatura Bilgileri, VUK md. 231/5) |
...
| `products` | Ürün partileri; benzersizlik `(code, production_year, production_week, COALESCE(supplier,''))` (tarihli) veya `(code)` (tarihsiz "temel" satır); `barcode` (`(tenant_id, barcode)` PARÇALI benzersiz, NULL olabilir), `product_type`, `width_mm`/`profile_pct`/`rim_diameter` (Ebat'ın yapılandırılmış hâli, `size_desc`'e ek), `tread_depth_mm` (İkinci El Lastik — bkz. Bölüm 9, Katalog Alanları) |
...
| `iyzico_checkout_sessions` | Abonelik (Bölüm 20) — checkout/switch-plan başlatılırken dönen `token`'ı `tenant_id`'ye eşler (`token` PK); callback, iyzico'nun hiç döndürmediği `conversationId` yerine bunu kullanır |
| `iyzico_webhook_events` | Abonelik (Bölüm 20) — `order_reference_code` (PK) ile webhook idempotency; aynı olay tekrar teslim edilirse dönem sonu ikinci kez uzatılmaz |
| `iyzico_payments` | Abonelik (Bölüm 20) — `payment_id` (PK) → `tenant_id` eşlemesi; IFN (dolandırıcılık) bildirimi yalnızca `paymentId` taşıdığından gerekli |
| `billing_events` | Abonelik/Süper Admin (Bölüm 17/20) — faturalandırma olay günlüğü: `tenant_id` (nullable), `event_type`, `detail`, `created_at`; webhook/IFN/repricing/tutarlılık düzeltmesi gibi olayları Süper Admin'de görünür kılar |
...
| GET | `/api/products/barcode` | Barkoda göre firmanın kendi geçmiş ürünlerinden otomatik doldurma (bkz. Bölüm 9, Katalog Alanları) |
...
| GET/POST | `/api/super-admin/tenants` | Süper Admin: tüm firmaları (+ güncel USD/TRY kuru) listele / `provisionTenant()` ile yeni firma oluştur, her zaman `billing_status='exempt'` (`role: super_admin`, bkz. Bölüm 17) |
| PATCH/DELETE | `/api/super-admin/tenants/:id` | Süper Admin: firmayı Aktif/Pasif yap / TÜM verisiyle kalıcı olarak sil (bkz. Bölüm 17) |
| GET | `/api/super-admin/tenants/:id/payments` | Süper Admin: tenant'ın iyzico Raporlama Servisi'nden tam ödeme geçmişi (komisyon, blokaj/hesaba geçiş durumu, fraudStatus dahil, bkz. Bölüm 17) |
| GET | `/api/super-admin/revenue` | Süper Admin: seçili aya ait GERÇEK platform geliri (`?month=YYYY-MM`, iyzico Raporlama Servisi'nden gün gün toplanır, bkz. Bölüm 17) |
| GET | `/api/super-admin/billing-events` | Süper Admin: son 100 faturalandırma olayı (`billing_events`, bkz. Bölüm 17) |
| GET/POST | `/api/super-admin/consistency-check` | Süper Admin: DB↔iyzico abonelik tutarlılık denetimi ve tek-tık düzeltme (bkz. Bölüm 17/20) |
| GET | `/api/company-info` | Oturum açmış herkes — İş Emri gibi belgelerde gösterilecek firma adı/adres/telefon/vergi bilgisi + logo/kaşe URL'leri (bkz. Bölüm 4) |
| POST/DELETE | `/api/company-info/assets` | Admin — Firma Logosu/Panel Logosu/Firma Kaşesi yükle/sil (Vercel Blob, bkz. Bölüm 15) |
| GET | `/api/billing/plans` | Abonelik: Aylık/Yıllık planın canlı iyzico fiyatı + güncel TCMB kuru (bkz. Bölüm 20) |
| POST | `/api/billing/checkout` | Abonelik: yeni abonelik başlat, iyzico Checkout Form döner (admin, eşzamanlılık kilidi + fatura bilgisi/sözleşme onayı zorunlu, bkz. Bölüm 20) |
| POST | `/api/billing/switch-plan` | Abonelik: Aylık↔Yıllık geçiş (prorasyonsuz, yalnızca yenilemeye 3 gün kala, bkz. Bölüm 20) |
| POST | `/api/billing/cancel` | Abonelik: iptal — iyzico'da anında, erişim ödenmiş dönem sonuna kadar sürer (bkz. Bölüm 20) |
| GET/POST | `/api/billing/callback` | Abonelik: iyzico Checkout Form'un yönlendirdiği geri çağırma ucu, aboneliği aktive eder (bkz. Bölüm 20) |
| GET/PUT | `/api/billing/invoice-info` | Abonelik: Fatura Bilgileri (VKN/TCKN, Vergi Dairesi vb.) oku/kaydet — hem `/admin/billing` hem Genel Ayarlar bunu kullanır (bkz. Bölüm 15/20) |
| POST | `/api/webhooks/iyzico` | Kimlik doğrulamasız (imza ile korunur) — iyzico abonelik yenileme başarılı/başarısız olayları (bkz. Bölüm 20) |
| POST | `/api/webhooks/iyzico-fraud/:secret` | Kimlik doğrulamasız (gizli URL ile korunur) — IFN dolandırıcılık incelemesi bildirimi (bkz. Bölüm 20) |
| GET | `/api/cron/reprice-subscriptions` | Kimlik doğrulamasız (Bearer `CRON_SECRET` ile korunur) — Vercel Cron, günlük, yenilemeye 3 gün kala USD/TRY kur farkını yeni bir iyzico plan fiyatına yansıtır (bkz. Bölüm 20) |
...
/admin/billing                → Abonelik (SaaS faturalandırma, iyzico Checkout Form — bkz. Bölüm 20) (admin)
...
- `middleware.ts`, `/` ve `/admin/*` sayfa isteklerini korur — ama **kimlik doğrulama** olarak API rotalarını (`/api/*`) kapsamaz; her API dosyası kendi `getAuthUser()` kontrolünü kendisi yapar (token geçerliliği). Tüm `route.ts` dosyaları bu deseni takip eder. **Tek istisna faturalandırma kilididir** (bkz. Bölüm 20): kilitli bir tenant'ın sayfa yönlendirmesini atlayıp API'ye doğrudan erişmeye devam edebildiği bir denetimde bulunduktan sonra, `middleware.ts` artık `/api/*` isteklerinde de (birkaç istisna route grubu hariç) ayrıca bu KİLİT kontrolünü yapıp 402 döner — genel kimlik doğrulamadan bağımsız, dar kapsamlı ikinci bir katman.
